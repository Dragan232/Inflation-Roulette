...
class FlxUIButton extends FlxUITypedButton<FlxUIText> implements ILabeled implements IFlxUIButton {
	...
	public function new(X:Float = 0, Y:Float = 0, ?Label:String, ?OnClick:Void -> Void, ?LoadDefaultGraphics:Bool = true, ?LoadBlank:Bool = false, ?Color:FlxColor = FlxColor.WHITE) {
		...
		if (Label != null) {
			...
			label = new FlxUIText(0, 0, 80, Label, 16);
			label.setFormat(null, 16, 0x333333, FlxTextAlign.CENTER);
		}
	 ...
	 */
	public function setLabelFormat(?Font:String, Size:Int = 8, Color:FlxColor = FlxColor.WHITE, ?Alignment:FlxTextAlign, ?BorderStyle:FlxTextBorderStyle, BorderColor:FlxColor = FlxColor.TRANSPARENT, Embedded:Bool = true):FlxText {
		if (label != null) {
