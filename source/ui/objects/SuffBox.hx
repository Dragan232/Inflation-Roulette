...
class SuffBox extends FlxSpriteGroup {
	...
	public function new(x:Float, y:Float, ?width:Float = 640, ?height:Float = 480) {
		...
		bg = new FlxUI9SliceSprite(0, 0, Paths.getImagePath('ui/boxes/box2xBase'), bgRect, nineSlice, 0x11);
		bg.setGraphicSize(Std.int(width), Std.int(height));
		bg.updateHitbox();
		bg.color = bgColor;
		add(bg);

		outline = new FlxUI9SliceSprite(0, 0, Paths.getImagePath('ui/boxes/box2xOutline'), bgRect, nineSlice, 0x11);
