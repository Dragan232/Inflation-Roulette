...
// Stole this from Psych Engine lmao
...
class FileDialogHandler extends FlxBasic {
	...
	public function open(?defaultName:String = '', ?title:String = null, ?filter:Array<FileFilter> = null, ?onComplete:Void -> Void, ?onCancel:Void -> Void, ?onError:Void -> Void) {
		if (!completed) {
	...
	public function openDirectory(?defaultPath:String = null, ?title:String = null, ?onComplete:Void -> Void, ?onCancel:Void -> Void, ?onError:Void -> Void) {
		if (!completed) {
			throw new Exception('You must finish previous operation before starting a new one.');
		}
		...
		_fileRef.addEventListener(#if desktop Event.SELECT #else Event.COMPLETE #end, _currentEvent);
		_fileRef.browseEx(OPEN_DIRECTORY, defaultPath, title);
	}
